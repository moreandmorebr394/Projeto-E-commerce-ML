...
# 3. Configuração de Autenticação Segura (Conversão Automática de Senha)
auth_config = st.secrets["auth"]

# Converte as senhas do secrets.toml para hash diretamente na memória
hasher = stauth.Hasher([])
credentials = {"usernames": {}}

for user, data in auth_config["credentials"]["usernames"].items():
    raw_password = str(data["password"])
    
    # Se a senha no secrets.toml já for um hash bcrypt, mantém; se não, converte
    if raw_password.startswith("$2b$") or raw_password.startswith("$2a$"):
        hashed_password = raw_password
    else:
        hashed_password = hasher.hash(raw_password)
        
    credentials["usernames"][str(user)] = {
        "name": str(data["name"]),
...
try:
    authenticator.login(location="main", fields={"Form name": "Login - Dashboard Olist"})
except TypeError:
    authenticator.login("Login - Dashboard Olist", "main")
...
if st.session_state.get("authentication_status") is False:
    ...
    st.info("Por favor, insira suas credenciais para acessar o painel.")
    ...
    role_usuario = "Viewer"
    if username_logado and username_logado in credentials["usernames"]:
        role_usuario = credentials["usernames"][username_logado].get("role", "Viewer")

    # Menu Lateral
    st.sidebar.markdown(f"### 👤 {user_name}")
    st.sidebar.caption(f"**Perfil:** {role_usuario}")
    ...
    # Declaração das Abas
    tab_evolucao, tab_categorias, tab_logistica, tab_concentracao = st.tabs([
        "📈 Evolução Temporal", 
    ...
    # --- ABA 2: DESEMPENHO POR CATEGORIA ---
    with tab_categorias:
    ...
    # --- RECURSOS PARA EDITOR / ADMINISTRADOR ---
    ...
    if role_usuario == "Administrador":
        ...
        with c_a1:
            st.session_state["modo_manutencao"] = st.toggle(
                "Ativar Modo de Manutenção do Sistema", 
                value=st.session_state.get("modo_manutencao", False)
            )
        with c_a2:
            st.session_state["limite_req"] = st.number_input(
                "Limite de Requisições Simultâneas", 
                min_value=10, 
                max_value=1000, 
                value=st.session_state.get("limite_req", 100)
            )
